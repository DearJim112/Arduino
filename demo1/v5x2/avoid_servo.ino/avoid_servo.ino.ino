...
void run()     // 前进
{
  ...
  analogWrite(Left_motor_back,95);
...
float Distance_test()   // 量出前方距离 
{
  ...
  //Fdistance_final=lastDistance*0.7+0.3*Fdistance;
...
void front_detection()
{
  ...
  front_distance_filter=front_distance_filter*0.7+Front_Distance*0.3;
...
//    Serial.print("Right_Distance:");
//    Serial.print("front_distance_filter:");
//    if(Front_Distance < 30)//当遇到障碍物时
    if(front_distance_filter < 40)//当遇到障碍物时
    {
      brake(2);//先刹车
      back(2);//后退减速
      brake(2);//停下来做测距
